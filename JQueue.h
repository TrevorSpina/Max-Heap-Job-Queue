...
private:
    ...
    // build the heap by calling maxHeapify on each entry with a child
    void buildHeap();
    
    // satisfy max heap requirements by recursively bubbling down the heap and swapping
    void maxHeapify(int index);
    
    // swap 2 values in the heap by index
